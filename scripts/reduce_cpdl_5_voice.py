#!/usr/bin/env python3
"""Generate supported CPDL five-voice Gesualdo reductions."""
...
from gesualdo_reduction.reduction import reduce_to_quartet, reduce_to_quartet_plus_viole
...
def default_output_dir(target: str) -> Path:
    return Path("data") / "cpdl" / "5-voices" / "reductions" / target


def reduce_source(target: str, source_path: str, output_path: Path):
    if target == "string_quartet":
        return reduce_to_quartet(
            source_path,
    ...
    if target == "string_quartet_plus_viole":
        return reduce_to_quartet_plus_viole(source_path, out_path=output_path)
...
def main() -> int:
    ...
    parser.add_argument(
        "--target",
        choices=("string_quartet", "string_quartet_plus_viole"),
    ...
    output_dir = args.output_dir or default_output_dir(args.target)
            ...
            for candidate_row, candidate_parts in sources:
                try:
                    if output_path.exists() and not args.force:
                        ...
                        reduced = reduce_source(args.target, candidate_row["local_path"], output_path)
                        global_transposition = getattr(reduced.editorial, "globalTransposition", "")
